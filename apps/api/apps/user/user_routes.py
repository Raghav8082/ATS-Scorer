...
from apps.schemas.user import UserCreate, UserOut, UserLogin, TokenResponse
...
async def login_user(user: UserLogin, db: AsyncSession = Depends(get_db)):
    return await user_login(db=db, login_data=user)

@user_router.put("/update/{id}", dependencies=[Depends(get_current_user)])
